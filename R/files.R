...
#' @param bed a list lazyframrd
#' @returns a list of lazyframes
#' @examples collapse_strands(bedfiles)
#' @keywords internal
#' @noRd 
#' @import polars
collapse_strands <- function(bed) {
  ...
  joint <- pos$join(
    neg,
    left_on = c("chr", "end"), right_on = c("chr", "cStart"), how = "full", coalesce = TRUE
...
#' @param collapse boolean; destrand CpG sites or not
#' @returns a lazyframe
#' @examples read_files(bedfile, TRUE)
#' @keywords internal
#' @noRd 
#' @import polars
...
#' @param df a dataframe (collected lazyframe)
#' @param outpath path to output file
#' @returns a saved bed file
#' @examples save_files(dataframe, /some/path/file.bed)
#' @keywords internal
#' @noRd 
#' @import polars
